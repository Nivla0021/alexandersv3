...
// ------- PRODUCT TYPE (UPDATED TO MATCH MenuClient EXPECTATIONS) -------
type Product = {
  id: string;
  name: string;
  description: string;
  ...
  variants: {
    ...
    price: number; // ✅ Added this to match MenuClient
  }[];
};
...
async function getProducts(category?: string): Promise<Product[]> {
  try {
    ...
    const onlineProducts = (products ?? [])
      .filter((p: any) => {
        const isOnlineAvailable = p.productType === 'online' || p.productType === 'both';
        const hasOnlineVariant = p.variants?.some((v: any) => v.onlinePrice && v.onlinePrice > 0);
      ...
      .map((p: any) => ({
        ...
        variants: p.variants
          ...
          .map((v: any) => ({
            ...v,
            // ✅ Added price property that MenuClient expects
...
async function getCategories() {
  try {
    ...
    const res = await fetch(`${host}/api/products`, { cache: 'no-store' });
    const products = await res.json();
    ...
    const onlineCategories = (products ?? [])
      ...
      .filter((category: string, index: number, self: string[]) => category && self.indexOf(category) === index)
      .sort();
