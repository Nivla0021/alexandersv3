...
export default function CheckoutPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    deliveryAddress: '',
    orderNotes: '',
    paymentMethod: 'GCASH', // Changed from 'COD' to 'GCASH'
  });
  ...
  useEffect(() => {
    const checkDiscountStatus = async () => {
      ...
      if (!session?.user) {
        setCheckingDiscountStatus(false);  // ✅ Add this line
        return;
      }
  ...
  useEffect(() => {
    const fetchActiveQRCode = async () => {
      try {
        const response = await fetch('/api/gcash-qr-codes/active');
        const data = await response.json();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const orderData = {
        ...
        paymentMethod: 'GCASH', // Always set to GCASH
  ...
  const handleGcashVerification = async () => {
    ...
    if (!gcashReference.trim()) {
      toast.error('Please enter the GCash reference number', {
        description: 'The reference number is required to verify your payment',
        duration: 4000,
      });
      return;
    }
    ...
    if (!receiptFile) {
      toast.error('Please upload your payment receipt', {
        description: 'A screenshot of your GCash payment is required',
        duration: 4000,
      });
      return;
    }
    ...
    // Show loading toast
    const loadingToast = toast.loading('Submitting payment verification...', {
    ...
    try {
      ...
      formData.append('gcashReference', gcashReference.trim());
      ...
      toast.success('Payment verification submitted!', {
        description: 'Your payment is being verified. We will process your order once confirmed.',
        duration: 5000,
        action: {
          label: 'View Order',
          onClick: () => router.push(`/order-confirmation/${currentOrder.orderNumber}`),
        },
      });
      ...
      toast.error('Verification failed', {
        description: err?.message || 'Failed to submit payment verification. Please try again.',
        duration: 5000,
        action: {
          label: 'Retry',
...
if (showGcashQR && currentOrder) {
  ...
  const downloadQRCode = () => {
    ...
    link.href = activeQRCode.imageUrl;
    link.download = `GCash-QR-${activeQRCode.name.replace(/\s+/g, '-').toLowerCase()}-Order-${currentOrder.orderNumber}.jpg`;
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 mb-8 max-w-md mx-auto">
                ...
                {activeQRCode ? (
                  <>
                    <img 
                      ...
                      alt={activeQRCode.name || "GCash QR Code"} 
                  ...
                  <div className="w-64 h-64 mx-auto flex items-center justify-center bg-gray-100 rounded-lg">
                    <p className="text-gray-500 text-sm">No active QR code available</p>
                  </div>
                )}
            ...
            <div className="space-y-3 mb-8">
              <button
                ...
                className="w-full py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center disabled:bg-gray-400 disabled:cursor-not-allowed"
              >
            ...
            <div className="mt-8 pt-6 border-t">
              ...
              <div className="mb-6">
                ...
                <input
                  ...
                  placeholder="Enter GCash reference no. from your payment"
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  disabled={verificationLoading}
                />
                <p className="text-xs text-gray-500 mt-1">
                  Find this in GCash after payment (e.g., ABC123456789)
                </p>
              </div>
                ...
                {!receiptPreview ? (
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-600">
                      Click to upload screenshot of your GCash payment
                    </p>
                    <p className="text-xs text-gray-500 mt-1">
              ...
              <div className="mt-6 space-y-3">
                <button
                  type="button"
                  onClick={handleGcashVerification}
                  disabled={verificationLoading || !gcashReference.trim() || !receiptFile}
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <ol className="list-decimal pl-5 space-y-2 text-gray-700">
                    ...
                    <li>Upload the screenshot above with your reference number</li>
                  </ol>
                </div>
  ...
  // Regular checkout form - now only showing GCASH as payment method
  return (
    <div className="min-h-screen flex flex-col">
      <Header />
      <main className="flex-1 bg-gradient-to-b from-amber-50 to-white">
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-md p-6 space-y-6">
                ...
                {/* Payment Method - Now only showing GCash */}
                <div>
                  <label className="block text-sm font-semibold text-amber-900 mb-2">
                    Payment Method *
                  </label>
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                    <div className="flex items-center">
                      <input
                        ...
                        value="GCASH"
                        checked={true}
                        onChange={() => {}} // No-op since it's the only option
                        className="mr-3"
                      />
