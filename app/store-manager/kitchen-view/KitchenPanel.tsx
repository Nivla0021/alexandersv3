...
export default function AdminOrdersPage() {
  ...
  // Loading state for individual order updates
                ...
                {todayOrders.length === 0 ? (
                    ...
                    <div className="grid 2xl:grid-cols-5 xl:grid-cols-3 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-2 grid-cols-1 gap-6">
                        {todayOrders.map((order) => {
                          const nextStatus = getNextStatuses(order)[0];
                          const isUpdating = updatingOrderId === order.id;
                          const buttonText = nextStatus ? nextStatus.charAt(0).toUpperCase() + nextStatus.slice(1) : "";
                                    ...
                                    <div className="mt-4 flex flex-col gap-2">
                                      ...
                                      {nextStatus && (
                                        <button
                                          ...
                                          disabled={isUpdating}
                                          className="xl:text-[14px] sm:text-[13.5px] text-[13px] text-white/85 hover:text-white py-2 bg-[#0ba574] hover:bg-[#01865C] rounded-lg cursor-pointer disabled:bg-[#0ba574]/70 disabled:cursor-not-allowed relative flex items-center justify-center"
                                        >
                                          {isUpdating ? (
                                            <>
